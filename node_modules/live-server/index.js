		...
		if (LiveServer.logLevel >= 1) {
			if (serveURL === openURL)
				if (serveURLs.length === 1) {
					console.log(("Serving \"%s\" at %s << Ctrl + Click").green, root, serveURLs[0]);
				} else {
					console.log(("Serving \"%s\" at\n\t%s << Ctrl + Click").green, root, serveURLs.join("\n\t"));
				}
			else
				console.log(("Serving \"%s\" at %s (%s) << Ctrl + Click").green, root, openURL, serveURL);
		}
